...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-f9bc0c5c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-f9bc0c5c">'+'<span class="tile-face-f9bc0c5c tile-front-f9bc0c5c"></span>'+'<span class="tile-face-f9bc0c5c tile-back-f9bc0c5c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-f9bc0c5c")){return;}
var back=tile.querySelector(".tile-back-f9bc0c5c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-f9bc0c5c");if(isMine){tile.classList.add("boom-f9bc0c5c");}
if(dim){tile.classList.add("dimmed-f9bc0c5c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-f9bc0c5c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-f9bc0c5c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-f9bc0c5c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-f9bc0c5c");void panelEl.offsetWidth;panelEl.classList.add("jolt-f9bc0c5c");}
endRound();}
